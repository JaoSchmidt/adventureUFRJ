...
jogador controle1(jogador p,int c){ //aqui é onde ocorre o controle do player 1;
    ...
    if(c==32){//barra de espaço
        if(p.direcao=='^'){
            //tiroacimax eh função
            p.tirocimax[p.qtiroscima]=p.x+1;//esses dois fazem o tiro surgir nas coord do player
            p.tirocimay[p.qtiroscima]=p.y-1;//y-1,x-2,x+1,etc dizem sobre a posição que o tiro surge
            //em função do tamanho do personagem, lembrando que ele é "\O/"
...
/*jogador controle2(jogador p,int c){ //aqui é onde ocorre o controle do player 2;
    ...
    if(c=='0'){
        if(p.direcao=='^'){
            p.qtiroscima++;
            p.tirocimax[p.qtiroscima]=p.x+3;
            p.tirocimay[p.qtiroscima]=p.y;
            }
        if(p.direcao=='<'){
            p.qtirosesquerda++;
        ...
        if(p.direcao=='v'){
            p.qtirosbaixo++;
            p.tirobaixox[p.qtiroscima]=p.x+3;
            p.tirobaixoy[p.qtiroscima]=p.y;
            }
        if(p.direcao=='>'){
            p.qTirosDireita++;
            p.tirodireitax[p.qTirosDireita]=p.x+3;
            p.tirodireitay[p.qTirosDireita]=p.y;
        }
    }
    return p;
...
void desenhaplayer(jogador p,int num,int direcao){//desenha o jogador no terminal
    mvprintw(0,p.x,"%d %d",p.qtiroscima,p.qtiroscima);
